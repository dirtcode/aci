...
pacman -S --noconfirm sudo openssh openssl iw wpa_supplicant zsh zsh-completions \
  ...
  rofi pavucontrol alsa-utils acpi sysstat scrot unrar wget networkmanager
...
#####################################
# set host name
echo "tntclient" > /etc/hostname
echo "127.0.0.1   localhost.localdomain   localhost tntclient" > /etc/hosts
echo "::1         localhost.localdomain   localhost tntclient" >> /etc/hosts
...
sed "s/exec.*/exec startkde/g" /etc/X11/xinit/xinitrc > /home/dieter/.xinitrc
